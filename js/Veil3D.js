 ...
 */
 ...
 VEIL.Veil3D = function(name) {
	...
	this.createPrimaryElements = function() {
		...
		var colorSpeed = VEIL.Params.colorSpeed;
		var colorRange = VEIL.Params.colorRange;
	...
	this.onDocumentMouseDown = function(event) {
		...
		mouse.x = ( event.clientX / (window.innerWidth-VEIL.Params.guiWidth) ) * 2 - 1;
	...
	this.parse = function() {
		...
		this.base.rotation.y += VEIL.Params.orbitSpeed;

		var centerSpeed = VEIL.Params.centerSpeed*.1;
		var noiseSpeed = VEIL.Params.noiseSpeed;
		var noiseAmount = VEIL.Params.noiseAmount;
		var noiseIntensity = VEIL.Params.noiseIntensity;
		var multiplier = VEIL.Params.multiplier;
		var centerRadius = VEIL.Params.centerRadius;
		var centerOffset = VEIL.Params.centerOffset;

		var waterHeight = VEIL.Params.waterHeight;
		var outerRadius = VEIL.Params.radius;
		var radiusRange = VEIL.Params.radiusRange;
		var wrapAmount = VEIL.Params.wrapAmount;
		var spikes = 0;
		var maxHeight = 0;

		var heightCountIncrement = (multiplier*2-multiplier)
		var heightOffset = VEIL.Params.heightOffset;
		var maxHeightRange = VEIL.Params.maxHeightRange;
		...
		for(j = 0; j < this.totalVerticesV; j++) {
			for(i = 0; i < this.totalVerticesH; i++) {
				...
				// spikes calculation
				outerRadius = VEIL.Params.radius - j/(this.totalVerticesV-1)*radiusRange * VEIL.Params.radius;
		...
		// adjusts color
		this.colorOffset += VEIL.Params.colorSpeed;
		var colorRange = VEIL.Params.colorRange;
		...
		outerRadius = VEIL.Params.radius - VEIL.Params.radius*radiusRange;
		for(j = 0; j < this.totalVerticesH; j++) {
			...
			// spikes calculation
			outerRadius = VEIL.Params.radius*1.125 - radiusRange * VEIL.Params.radius;
			spikes = outerRadius - fold*outerRadius;
...
VEIL.Veil3D.prototype = new UNCTRL.BoilerPlate();
VEIL.Veil3D.prototype.constructor = VEIL.Veil3D;